...
from typing import Dict, Iterable, List, Optional, Tuple, Mapping, Any, Union
...
from jax import Array, device_get, device_put, jit
...
def collect_stats(
        ...
        train_iter: DataLoader and Iterable[Tuple[List[str], Dict[str, np.ndarray]]],
        valid_iter: DataLoader and Iterable[Tuple[List[str], Dict[str, np.ndarray]]],
        output_dir: Path,
        write_collected_feats: bool,
        ngpu: Optional[int] = None,  # not used currently
    ...
    @jit
    def apply_collect_feats(batch: Union[Array, np.ndarray]) -> Dict[str, Any]:
        return model.apply(variables, method=model.collect_feats, training=True, **batch)
    ...
    for itr, mode in zip([train_iter, valid_iter], ["train", "valid"]):
        ...
        with DatadirWriter(output_dir / mode) as datadir_writer:
            for iiter, (keys, batch) in enumerate(itr, 1):
                ...
                data = apply_collect_feats(batch)
                data = device_get(data)
                for key, v in data.items():
                    for i, (uttid, seq) in enumerate(zip(keys, v)):
