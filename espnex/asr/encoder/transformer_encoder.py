...
from espnex.models.transformer.attention import MultiHeadDotProductAttention
...
class TransformerEncoder(AbsEncoder):
    ...
    def __call__(
        ...
        dense, mha = map(inject, (Dense, MultiHeadDotProductAttention))
        ...
        if self.input_layer == "linear":
            xs_pad = Sequential(
                [
                    dense(self.output_size, kernel_init=self.kernel_init),
                    LayerNorm(),
            ...
            xs_pad, ilens = get_default_conv2d_subsampling(
                self.output_size, ratio, self.dropout_rate, kernel_init=self.kernel_init, deterministic=deterministic, name='embed'
            )(xs_pad, ilens)
            ...
            xs_pad = dense(self.output_size, name='embed')(xs_pad)
        ...
        positionwise_layer = partial(positionwise_layer, *positionwise_layer_args)
        ...
        for layer_idx in range(self.num_blocks):
            encoder_layer = EncoderLayer(
                mha(num_heads=self.attention_heads,
