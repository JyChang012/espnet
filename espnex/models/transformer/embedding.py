...
def sinusoidal_init(
    ...
    def init_espnet(shape: Sequence[int], dtype=np.float32) -> Array:
        ...
        div_term = min_scale * np.exp(np.arange(0, d_feature, 2) * scale_factor)  # (d_feature // 2,)
...
class AddPositionalEncoding(Module):
    ...
    def __call__(self, x: Array, deterministic: Optional[bool] = None) -> Array:
        ...
        pos_emb = sinusoidal_init(max_len=max_len, init_type=self.init_type)(pos_emb_shape)
