...
from espnex.models.transformer.subsampling import defaults_settings, get_default_conv2d_subsampling
...
def test_subsampling(in_shape, ratio, odim, deterministic, jit):
    model = get_default_conv2d_subsampling(odim, ratio, .5)
    x = jnp.zeros(in_shape)
    mask = jax.random.bernoulli(key, .5, in_shape[:-1])
